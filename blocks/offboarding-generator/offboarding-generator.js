...
function createInput(id, type, label, placeholder, value, tooltip) {
  ...
  input += '<input type="' + type + '" id="' + id + '"' + (type === 'text' || type=== 'email' || type === 'tel' ? ' placeholder="' + placeholder + '"' : null ) + (value ? ' value="' + value + '"' : null) + '/>';
  input += '</div>';
  return input;
}
...
function templateTone() {
  const templates = templateOptions.find(item => item.value = sessionStorage.getItem('generator-template'));
  let output = '<div class="tone-selection">';
  for (const [key, val] of Object.entries(templates.templates)) {
    const label = key.charAt(0).toUpperCase() + key.slice(1);
    const selected = (key === 'formal' ? 'checked' : null);
    output += '<input type="radio" name="select-tone" id="' + key + '" value="' + key + '" ' + selected + '/><label for="' + key + '">' + label + '</label>';
  }
  output += '</div>';
  output += '<div class="tone-template">';
  output += '<div id="template-preview"></div>';
  output += '</div>';
  output += '<nav>';
  output += '<button data-step="2" data-prev class="button button--outline">Back</button>';
  output += '<button data-step="2" class="button" id="lead-gen">Get my template</button>';
  output += '</nav>';
  return output;
}
...
function generateInputs(type) {
  ...
  switch(type) {
    case 'resignation-acknowledgement':
...
export default function decorate(block) {
  ...
  document.getElementById('select-template').addEventListener('click', (el) => {
    const selectedTemplate = document.getElementById('template-options').value;
    const templates = templateOptions.find(item => item.value === selectedTemplate);
    document.getElementById('template-preview').innerHTML = templates.templates[sessionStorage.getItem('generator-tone')];
  ...
  toneSelection.forEach(radio => radio.addEventListener('change', () => {
    ...
    const template = sessionStorage.getItem('generator-template');
    const templates = templateOptions.find(item => item.value === template);
    document.getElementById('template-preview').innerHTML = templates.templates[tone];
  }));
