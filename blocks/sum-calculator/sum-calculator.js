 ...
 * @param {string} formId
 */
function appendCalcResultToDom(totalCost, formId) {
  const form = document.getElementById(formId);
  const costParagraph = form.querySelector('.calc-result');
...
function formSubmitHandler(form) {
  ...
  elements.forEach((element) => {
    if (element.tagName === 'INPUT') {
      compensationValues.push(element.value);
    }
  });

  const totalCost = calcTotal(compensationValues);

  appendCalcResultToDom(totalCost, form.id);
}
...
function createCalcResultHtml(resultText) {
...
function createForm(fields, resultText) {
  ...
  form.append(createCalcResultHtml(resultText));
...
export default async function decorate(block) {
  let source = '';
  let resultText = 'This is the total.';
  const rows = Array.from(block.children);
  rows.forEach((row) => {
    const columns = row.children;
    const key = columns[0].innerText.toLowerCase();
  ...
  const data = await fetchData(`${source}.json`);

  data.forEach((item) => {
    formFields.push(item);
  });

  block.append(createForm(formFields, resultText));
