...
async function runExperiment(experiment, instantExperiment) {
  const ued = import ('./ued.js');
  const usp = new URLSearchParams(window.location.search);
  ...
  await replaceInner(pages[0], main);
  main.classList.add(experimentConfig.id);
  main.classList.add(experimentConfig.selectedVariant);
}
